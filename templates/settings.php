    ...
    <fieldset class="personalblock">
        ...
        <p>
            ...
            <input type="password" id="wordpress_db_password" name="wordpress_db_password" 
                value="<?php echo $_['wordpress_db_password']; ?>" />

            
        </p>
		<p><?php echo $l->t('Required fields for wordpress');?></p>
      <label for="wordpress_db_prefix"><?php echo $l->t('DB Prefix');?></label>
            <input type="text" id="wordpress_db_prefix" name="wordpress_db_prefix" 
                value="<?php echo $_['wordpress_db_prefix']; ?>" />
        
            <!--label for="wordpress_db_user"><?php echo $l->t('Hash Salt');?></label-->
            <input type="hidden" id="wordpress_hash_salt" name="wordpress_hash_salt" 
                value="<?php echo $_['wordpress_hash_salt']; ?>" />
        </p>
        <p><?php echo $l->t('Custom settings');?></p>
            ...
            <input type="radio" id="wordpress_have_to_be_logged" name="wordpress_have_to_be_logged" 
                value="1" <?php if($_['wordpress_have_to_be_logged']=='1'){ echo'checked'; } ?> /></label>
                
                <label for="wordpress_have_to_be_logged"><?php echo $l->t('All users are visible');?>
            <input type="radio" id="wordpress_have_to_be_logged" name="wordpress_have_to_be_logged" 
                value="0" <?php if($_['wordpress_have_to_be_logged']=='0'){ echo'checked'; } ?> /></label>
        </p>
        
        <p><?php echo $l->t('Global group');?></p>
      		<label for="wordpress_global_group"><?php echo $l->t('Create a group for all Wordpress users');?>
            <input type="text" id="wordpress_global_group" name="wordpress_global_group" 
                value="<?php echo $_['wordpress_global_group']; ?>" /></label>
        </p>
