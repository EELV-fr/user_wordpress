...
function publierdocument(domain,titre,lien) {
	 var pop=window.open(parent.location.protocol+'//'+domain+'/wp-admin/press-this.php?u=&t='+titre+'&s='+t('user_wordpress','File shared from')+' '+location.host+'%0D%0D'+lien+'&v=4','pop','width:400,height:500');
  $('.wordpresshare').hide();
}
...
function publishdialog(sites_list,type,itemSource,file){
	...
	$.ajax({
        type: 'POST',
        url: OC.linkTo('', 'index.php/core/ajax/share.php?fetch=getItem&itemType='+type+'&itemSource='+itemSource+'&checkReshare=false&checkShares=true&isWp=1'),
        dataType: 'json',
        async: false,
        success: function (k_file) {
        	file_item = k_file['data']['shares'];
			for(var the_item in file_item){
				var item_link = parent.location.protocol+'//'+location.host+OC.linkTo('', 'public.php')+'?service=files%26t='+file_item[the_item]['token']+'%26f='+file;
				  
				var html='<div id="wordpresshare" class="wordpresshare"><ul>';
				for(var i=0 ; i<sites_list.length ; i++){
				  html+='<li data-domain="'+sites_list[i]['domain']+'">'+sites_list[i]['domain']+'</li>'; 
				}
				html+='</ul></div>';
				...
				$('#wordpresshare li').click(function(){
					publierdocument($(this).data('domain'),file,item_link);
				});
